...
public:
	...
	/// @returns True if success, false if failed
	bool DownloadfromServerID(uint64_t id,eastl::string16 dst);
	/// 
	...
	/// @returns True if success, false if failed
	bool GetKeyfromServerID(uint64_t id,eastl::string16 dst, ResourceKey& key);
	bool GetKeyfromServerID(uint64_t id, ResourceKey& key); // Unused function, will be looked at more once we know more about address 0x54e460 (March 2017)
